...
workspace_path_b2 <- here::here("data/20180726_SATVI-TBRU_Memory_B2/20180726_SATVI-TBRU_Mem_B2.xml")
fcs_path_b2 <- here::here("data/20180726_SATVI-TBRU_Memory_B2/20180726_SATVI-TBRU_Memory_B2")
gs_out_dir_b2_krystle <- here::here("out/GatingSets/20180906_GatingSet_Batch2_Krystle")
gs_out_dir_b2_fmo_krystle <- here::here("out/GatingSets/20180906_GatingSet_Batch2_FMO_Krystle")

gs_out_dir_b2_autogate <- here::here("out/GatingSets/20180906_GatingSet_Batch2_AutoGate")
gs_out_dir_b2_fmo_autogate <- here::here("out/GatingSets/20180906_GatingSet_Batch2_FMO_AutoGate")

gating_plots_dir_b2 <- here::here("out/GatingPlots/20180906_Batch2")

ws_b2 <- open_flowjo_xml(workspace_path_b2)
...
gs_b2 <- parseWorkspace(ws_b2, name = "Concatenated Samples",
...
gs_b2_markernames <- c("Time", "FSC-A", "FSC-H", "SSC-A", "SSC-H", "CD8b", "L/D", "CD45RA", "CD3", 
                       "Unloaded CD1b", "CD4", "CD56", "GMM_R660", "CD14/CD19", "CCR7", "GMM_V655", 
                       "HLA-DR", "TRAV1-2", "CD38")
names(gs_b2_markernames) <- c("Time", "FSC-A", "FSC-H", "SSC-A", "SSC-H", "<B710-A>", "<B515-A>", 
                              "<G780-A>", "<G610-A>", "<G575-A>", "<R780-A>", "<R710-A>", "<R660-A>", 
                              "<V780-A>", "<V710-A>", "<V655-A>", "<V610-A>", "<V510-A>", "<V450-A>")
markernames(gs_b2) <- gs_b2_markernames
markernames(gs_b2)
pData(parameters(getData(gs_b2[[1]])))[,c(1,2)]

# Save Krystle's version of the gating as a Gating Set for now.
save_gs(gs_b2, gs_out_dir_b2_krystle)

fmo_b2 <- parseWorkspace(ws_b2, name = "FMO Controls",
                         keywords = keywords2import,
                         path = fcs_path_b2)
pData(fmo_b2)$filename <- sapply(rownames(pData(fmo_b2)), function(x) {
  gsub("/.+/", "", description(getData(fmo_b2[[x]]))$FILENAME)
}, USE.NAMES = F)
pData(fmo_b2)$rowname <- rownames(pData(fmo_b2))
pData(fmo_b2)

# FMO: Set marker names where needed
pData(parameters(getData(fmo_b2[[1]])))[,c(1,2)]
fmo_b2_markernames <- c("Time", "FSC-A", "FSC-H", "SSC-A", "SSC-H", "CD8b", "L/D", "CD45RA", "CD3", 
                        "Unloaded CD1b", "CD4", "CD56", "GMM_R660", "CD14/CD19", "CCR7", "GMM_V655", "HLA-DR", "TRAV1-2", 
                        "CD38")
names(fmo_b2_markernames) <- c("Time", "FSC-A", "FSC-H", "SSC-A", "SSC-H", "<B710-A>", "<B515-A>", 
                               "<G780-A>", "<G610-A>", "<G575-A>", "<R780-A>", "<R710-A>", "<R660-A>", 
                               "<V780-A>", "<V710-A>", "<V655-A>", "<V610-A>", "<V510-A>", "<V450-A>")
markernames(fmo_b2) <- fmo_b2_markernames
markernames(fmo_b2)
pData(parameters(getData(fmo_b2[[1]])))[,c(1,2)]
...
save_gs(fmo_b2, gs_out_dir_b2_fmo_krystle)

# Lymphocytes gate stats: over 10,000? (Yes)
getPopStats(gs_b2, subpopulation = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")

# Make a couple example plots of Krystle's gating
png(filename = file.path(gating_plots_dir_b2, "20180906_Krystle_GatingTree.png"), width = 1000, height = 1000)
plot(gs_b2, fontsize = 20)
dev.off()

png(filename = file.path(gating_plots_dir_b2, "20180906_Krystle_plotGate_Example.png"), width = 1000, height = 1000)
plotGate(gs_b2[[1]])
dev.off()
...
# Time
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_%s.png", "Time_vs_FSC-A",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "Time", y = "FSC-A"),
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_%s.png", "Time_vs_CD56",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "Time", y = "CD56"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") +
  facet_grid(Individual ~ TimePoint)
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_%s.png", "Time_vs_CD4",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "Time", y = "CD4"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") +
  facet_grid(Individual ~ TimePoint)
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_%s.png", "Time_vs_CD8b",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "Time", y = "CD8b"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") +
  facet_grid(Individual ~ TimePoint)
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_%s.png", "Time_vs_GMM_V655",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "Time", y = "GMM_V655"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") +
  facet_grid(Individual ~ TimePoint)
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_%s.png", "Time_vs_GMM_R660",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "Time", y = "GMM_R660"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") +
  facet_grid(Individual ~ TimePoint)
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_%s.png", "Time_vs_Unloaded_CD1b",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "Time", y = "Unloaded CD1b"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") +
  facet_grid(Individual ~ TimePoint)
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_%s.png", "Time_vs_CD3",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "Time", y = "CD3"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") +
  facet_grid(Individual ~ TimePoint)
...
# CD56
ggcyto(gs_b2[[1]], mapping = aes(x = "CD3", y = "CD56"), subset = "root") + geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument")
# ggcyto(gs_b2[[1]], mapping = aes(x = "CD3", y = "CD56"), subset = "CD3-") + geom_hex(bins=120) +
#   ggcyto_par_set(limits = "instrument")
...
add_pop(gs_b2, alias = "CD56+", pop = "+", parent = "root",
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_vs_CD3_%s.png", "CD56+",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, aes(x = "CD3", y = "CD56"), subset = "root") + geom_hex(bins=120) +
...
# HLA-DR and CD38
ggcyto(gs_b2[[1]], mapping = aes(x = "CD38", y = "HLA-DR"), subset = "/CD3-") + geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument")

add_pop(gs_b2, alias = "CD38+", pop = "+", parent = "/CD3-",
        dims = "CD38", gating_method = "mindensity2", gating_args = "gate_range=c(1800,2100)",
        mc.cores = 14, parallel_type = "multicore", collapseDataForGating = T, groupBy = "Individual")

add_pop(gs_b2, alias = "HLA-DR+", pop = "+", parent = "/CD3-",
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_%s.png", "CD38_and_HLA-DR",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "CD38", y = "HLA-DR"), subset = "/CD3-") + geom_hex(bins=120) +
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_on_Lymphocytes_%s.png", "CD38_and_HLA-DR",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "CD38", y = "HLA-DR"), subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") + geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") + geom_gate(c("/CD38+", "/HLA-DR+")) +
  labs(title = "CD38 and HLA-DR    collapsed by Individual", caption = batch) + facet_grid(Individual ~ TimePoint)
dev.off()

# TRAV1-2
ggcyto(gs_b2[[1]], mapping = aes(x = "FSC-A", y = "TRAV1-2"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument")

add_pop(gs_b2, alias = "TRAV1-2+", pop = "+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes",
...
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_%s.png", "TRAV1-2",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "FSC-A", y = "TRAV1-2"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
  ggcyto_par_set(limits = "instrument") +
  facet_grid(Individual ~ TimePoint) +
...
ggcyto(gs_b2[[1]], mapping = aes(x = "CD8b", y = "CD4"),
...
quadGate_seq_mimicer(my_gs = gs_b2, parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes",
                     dim1 = "CD4", dim2 = "CD8b",
                     dim1_gating_args = "gate_range=c(1000,2800)",
                     dim2_gating_args = "gate_range=c(500,2000)")

plot(gs_b2, fontsize = 25, bool = T)

png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_%s.png", "CD4_and_CD8",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 1900, height = 1030)
ggcyto(gs_b2, mapping = aes(x = "CD4", y = "CD8b"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=120) +
...
flowWorkspace::add(gs_b2, eval(substitute(flowWorkspace::booleanFilter(v),
...
flowWorkspace::add(gs_b2, eval(substitute(flowWorkspace::booleanFilter(v),
                                          list(v = as.symbol("!/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4+&/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4-/CD8b+")))),
                   parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes", name = "CD4- CD8+")
flowWorkspace::recompute(gs_b2, "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")
...
add_pop(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b APC FMO")),
...
plotGate(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b APC FMO")),
         "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_R660+", xbins = 120)
# Copy the gate to the full stain sample
gmm_r660_gate <- getGate(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b APC FMO"))[[1]],
                         "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_R660+")
add(subset(fmo_b2, `SAMPLE ID` %in% c("Full Stain")), gmm_r660_gate,
    name = "GMM_R660+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")
recompute(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b APC FMO", "Full Stain")),
          "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")
plotGate(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b APC FMO", "Full Stain")),
         "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_R660+", xbins = 120)

# Rm("/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_R660+",
#    subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b APC FMO", "Full Stain")))

png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_%s.png", "GMM_R660_FMO",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 600, height = 400)
ggcyto(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b APC FMO", "Full Stain")),
...
# Just for Batch_2
pData(fmo_b2)$`SAMPLE ID`[which(pData(fmo_b2)$`SAMPLE ID` == "GMM CD1b BV650 FRMO")] <- "GMM CD1b BV650 FMO"

# Use tailgate on the "GMM CD1b APC FMO" sample and copy it over to the full stain sample
add_pop(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b BV650 FMO")),
        pop = "+", alias = "GMM_V655+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes",
        dims = "GMM_V655", gating_method = "tailgate",
        gating_args = "tol=0.1,bias=500", collapseDataForGating = F) # 
plotGate(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b BV650 FMO")),
         "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_V655+", xbins = 120)
# Copy the gate to the full stain sample
gmm_v655_gate <- getGate(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b BV650 FMO"))[[1]],
                         "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_V655+")
add(subset(fmo_b2, `SAMPLE ID` %in% c("Full Stain")), gmm_v655_gate,
    name = "GMM_V655+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")
recompute(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b BV650 FMO", "Full Stain")),
          "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")
plotGate(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b BV650 FMO", "Full Stain")),
         "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_V655+", xbins = 120)

# Rm("/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_V655+",
#    subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b BV650 FMO", "Full Stain")))

png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_%s.png", "GMM_V655_FMO",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 600, height = 400)
ggcyto(subset(fmo_b2, `SAMPLE ID` %in% c("GMM CD1b BV650 FMO", "Full Stain")),
...
add_pop(subset(fmo_b2, `SAMPLE ID` %in% c("Unloaded CD1b FMO")),
        ...
        gating_args = "tol=0.1,bias=350", collapseDataForGating = F) # 
plotGate(subset(fmo_b2, `SAMPLE ID` %in% c("Unloaded CD1b FMO")),
         "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/Unloaded_CD1b+", xbins = 120)
# Copy the gate to the full stain sample
gmm_unloaded_gate <- getGate(subset(fmo_b2, `SAMPLE ID` %in% c("Unloaded CD1b FMO"))[[1]],
                             "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/Unloaded_CD1b+")
add(subset(fmo_b2, `SAMPLE ID` %in% c("Full Stain")), gmm_unloaded_gate,
    name = "Unloaded_CD1b+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")
recompute(subset(fmo_b2, `SAMPLE ID` %in% c("Unloaded CD1b FMO", "Full Stain")),
          "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")
plotGate(subset(fmo_b2, `SAMPLE ID` %in% c("Unloaded CD1b FMO", "Full Stain")),
         "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/Unloaded_CD1b+", xbins = 120)

# Rm("/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/Unloaded_CD1b+",
#    subset(fmo_b2, `SAMPLE ID` %in% c("Unloaded CD1b FMO", "Full Stain")))

png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_%s.png", "Unloaded_CD1b_FMO",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
    width = 600, height = 400)
ggcyto(subset(fmo_b2, `SAMPLE ID` %in% c("Unloaded CD1b FMO", "Full Stain")),
       mapping = aes(x = "Unloaded CD1b", y = "GMM_V655"),
       subset = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes") +
  geom_hex(bins=240) +
...
flowWorkspace::add(gs_b2, getGate(gs_b2, "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/GMM_clean"), parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD3_clean", name = "GMM_clean")
flowWorkspace::recompute(gs_b2, "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes")

# GMM_clean count
gmm_clean_count <- dplyr::left_join(pData(gs_b2),
                                    getPopStats(gs_b2, subpopulation = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD3_clean/GMM_clean"),
                                    by = c("rowname" = "name"))[, c("SAMPLE ID", "Individual", "TimePoint",
                                                                    "SampleDate", "TimePoint_v2", "Day",
                                                                    "Count", "ParentCount")]
gmm_clean_count
write.table(x = gmm_clean_count, file = file.path(gating_plots_dir_b2,
...
ggcyto(gs_b2, mapping = aes(x = "CCR7", y = "CD45RA"),
...
# CD4+ CD8-
add_pop(gs_b2, alias = "CD45RA+", pop = "+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4+ CD8-",
        dims = "CD45RA", gating_method = "mindensity2", gating_args = "gate_range=c(1100,2200)",
        mc.cores = 14, parallel_type = "multicore", collapseDataForGating = T, groupBy = "Individual")
add_pop(gs_b2, alias = "CCR7+", pop = "+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4+ CD8-",
...
# CD4- CD8+
add_pop(gs_b2, alias = "CD45RA", pop = "+/-", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4- CD8+",
        dims = "CD45RA", gating_method = "mindensity2", gating_args = "gate_range=c(1400,1900)",
        mc.cores = 14, parallel_type = "multicore", collapseDataForGating = T, groupBy = "Individual")
add_pop(gs_b2, alias = "CCR7+", pop = "+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4- CD8+/CD45RA+",
        dims = "CCR7", gating_method = "mindensity2", gating_args = "gate_range=c(1000,2000)",
        mc.cores = 14, parallel_type = "multicore", collapseDataForGating = T, groupBy = "Individual")
add_pop(gs_b2, alias = "CCR7+", pop = "+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4- CD8+/CD45RA-",
        dims = "CCR7", gating_method = "mindensity2", gating_args = "gate_range=c(1800,2200)",
        mc.cores = 14, parallel_type = "multicore", collapseDataForGating = T, groupBy = "Individual")
png(filename = file.path(gating_plots_dir_b2,
                         sprintf("%s_gate_%s.png", "CCR7_and_CD45RA_on_CD4-CD8+",
                                 format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
...
add_pop(gs_b2, alias = "CCR7+", pop = "+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4- CD8-/CD45RA+",
        ...
        mc.cores = 14, parallel_type = "multicore", collapseDataForGating = T, groupBy = "Individual")
add_pop(gs_b2, alias = "CCR7+", pop = "+", parent = "/Live-CD3+/CD14-CD19-/S/CCR7 Keeper/CD38 Keeper/Lymphocytes/CD4- CD8-/CD45RA-",
        dims = "CCR7", gating_method = "mindensity2", gating_args = "gate_range=c(1200,1800)",
        mc.cores = 14, parallel_type = "multicore", collapseDataForGating = T, groupBy = "Individual")
png(filename = file.path(gating_plots_dir_b2,
...
getPopStats(gs_b2, subpopulations = c("Lymphocytes/Naive", "Lymphocytes/TEM", "Lymphocytes/TCM", "Lymphocytes/TEMRA"))
...
png(filename = file.path(gating_plots_dir_b2, "20180907_AutoGate_GatingTree.png"), width = 1000, height = 1000)
plot(gs_b2, fontsize = 25, bool = T)
dev.off()

# Save the GatingSets
save_gs(gs_b2, gs_out_dir_b2_autogate)
save_gs(fmo_b2, gs_out_dir_b2_fmo_autogate)
